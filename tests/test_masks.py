...
def test_get_mask_card_number_wrong_length(card_number: Any) -> None:
    with pytest.raises(ValueError):
...
def test_get_mask_account(account_number: Any, expected: Any) -> None:
    assert get_mask_account(account_number) == expected


def test_get_mask_empty_account() -> None:
    with pytest.raises(ValueError):
        get_mask_account()


@pytest.mark.parametrize('account_number', ['111', '111111111111111111111111111111111111111111111'])
def test_get_mask_account_wrong_length(account_number: Any) -> None:
    with pytest.raises(ValueError):
