...
def test_filter_by_state_with_EXECUTED(test_processing_list):
    assert filter_by_state(test_processing_list) == \
           [{'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
            {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'}]


def test_filter_by_state_with_CANCELED(test_processing_list):
    assert filter_by_state(test_processing_list, 'CANCELED') == \
           [{'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'},
            {'id': 615064591, 'state': 'CANCELED', 'date': '2018-10-14T08:21:33.419441'}]


@pytest.mark.parametrize('state, expected', [('EXECUTED', [
    {'id': 41428829, 'state': 'EXECUTED', 'date': '2019-07-03T18:35:29.512364'},
    {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572'}]),
                                             ('CANCELED', [
    {'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'},
    {'id': 615064591, 'state': 'CANCELED', 'date': '2018-10-14T08:21:33.419441'}])])
def test_filter_by_state(test_processing_list, state, expected):
    assert filter_by_state(test_processing_list, state) == expected


def test_filter_by_state_with_wrong_state(test_processing_list):
    with pytest.raises(ValueError):
        filter_by_state(test_processing_list, 'WRONG_STATE')


def test_sort_by_date_decreasing(test_processing_list):
...
def test_sort_by_date_increasing(test_processing_list):
