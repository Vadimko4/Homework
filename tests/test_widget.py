...
@pytest.mark.parametrize('account_card_information, expected', [('Visa Classic 6831982476737658', 'Visa Classic 6831 98** **** 7658'),
                                                                ('Visa Platinum 7000792289606361', 'Visa Platinum 7000 79** **** 6361'),
                                                                ('Счет 73654108430135874305', 'Счет **4305')])
def test_mask_account_card(account_card_information, expected):
    assert mask_account_card(account_card_information) == expected


def test_mask_empty_account_card():
...
def test_mask_wrong_card(account_card_information):
    with pytest.raises(ValueError):
...
def test_mask_wrong_account(account_card_information):
    with pytest.raises(ValueError):
        mask_account_card(account_card_information)
